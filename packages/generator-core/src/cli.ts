...
import { runAdaptProjectCommand } from './commands/adapt-project.js';
 ...
 *   adapt-project       --template-id X                 — adaptar projeto Angular aos padrões
...
function main(): void {
  ...
  try {
    switch (command) {
      ...
      case 'adapt-project':
        handleAdaptProject(args.slice(1), repoRoot);
        break;
...
function handleAdaptProject(args: string[], repoRoot: string): void {
  const templateId = getArg(args, '--template-id');

  if (!templateId) {
    console.error('Uso: npm run adapt-project -- --template-id <id>');
    process.exit(1);
  }

  const templatesDir = path.join(repoRoot, 'templates');
  runAdaptProjectCommand({ templateId, templatesDir }).catch((err) => {
    console.error(`\n❌ Erro: ${err instanceof Error ? err.message : err}`);
    process.exit(1);
  });
}
...
Comandos disponíveis:
  ...
  adapt-project --template-id <id>
    Adapta projeto Angular aos padrões do Template Lab usando IA.
    Refatora routing, marcação de blocos, resolução de steps, etc.
    Ex: npm run adapt-project -- --template-id plano-saude-base
